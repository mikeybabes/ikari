...
# Note: In Ikari Warriors arcade, the screen (tilemap) memory is mapped in RAM at address $C800-$CFFF.
...
def load_table(table_file):
    ...
    with open(table_file, 'rb') as f:
        raw = f.read()
        for i in range(1, len(raw)):  # start from third byte
...
def draw_chunks(tiles_data, map_data, palette, chunk_starts, chunks_per_image, base_output_file):
    ...
    for offset_index, chunk_start_index in enumerate(chunk_starts):
        print(f"Processing chunk {offset_index + 1}: table value = {chunk_start_index}")  # Debug output
        ...
        for index, tile_index in enumerate(chunk_data):
...
def main():
    if len(sys.argv) < 6:
        print("Usage: python ikari_map_plot.py tiles.bin map.bin palette.bin table.bin output.png [chunks_per_image]")
        return
